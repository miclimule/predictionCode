...
//		updateFile("predictionAjout.csv", "prediction.csv");
	...
	public static String getData(String[] langage) {
		int minLen = 0 ;
		int maxLen = 0 ;
		int compteurZeros = 0;
        int compteurUns = 0;
        minLen = langage[0].length();
		maxLen = langage[0].length();
		for (String string : langage) {
			if (minLen > string.length()) {
				minLen = string.length();
			}
		...
		vecteur.addAll(Arrays.asList(langage));
		codeLongeurFixe(vecteur);
    	codePrefixe(vecteur);
    	codeUnique(vecteur);
        codeFactorisable(vecteur);
        
		return "["+isLongeurFixe+","+isPrefixe+","+isUnique+","+isFactorisable+","+minLen+","+maxLen+","+langage.length+","+compteurUns+","+compteurZeros+"]";
	}
